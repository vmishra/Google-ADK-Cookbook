"""Loan officer FastAPI service (port 8007).

The customer-facing desk. Uses the credit_bureau peer — configured
via BUREAU_URL — as a remote scoring service. When the
`request_credit_report` tool is called, it HTTP-posts to the
bureau's /score endpoint; the tool_result that flows through this
server's SSE stream carries what the bureau returned.
"""
...
if not (_has_key or _use_vertex):
    print(
        "[loan-officer] WARNING: no GOOGLE_API_KEY / GEMINI_API_KEY and "
        "GOOGLE_GENAI_USE_VERTEXAI not true. Chat will fail.",
        flush=True,
    )
else:
    print(f"[loan-officer] auth ok · vertex={_use_vertex} · bureau={os.environ.get('BUREAU_URL', 'http://127.0.0.1:8017')}", flush=True)

from loan_officer import root_agent  # noqa: E402
from loan_officer.metrics import MetricsStore, TurnMetrics  # noqa: E402
from loan_officer.introspect import introspect  # noqa: E402


APP_NAME = "loan-officer"
USER_ID = "underwriter"

runner = InMemoryRunner(agent=root_agent, app_name=APP_NAME)
metrics = MetricsStore()
app = FastAPI(title="Small-business loan desk", version="1.0.0")
        ...
        async for event in runner.run_async(
            ...
            for part in (event.content.parts if event.content else []):
                ...
                if part.function_call:
                    ...
                    # A2A signal: when the desk calls request_credit_report,
                    # the portal can render a "→ bureau" peer indicator.
                    if part.function_call.name == "request_credit_report":
                        yield _sse({
                            "kind": "peer_call",
                            "author": author,
                            "peer": "credit_bureau",
                            "peer_url": os.environ.get("BUREAU_URL", "http://127.0.0.1:8017"),
                        })
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="127.0.0.1", port=int(os.environ.get("PORT", 8007)))
